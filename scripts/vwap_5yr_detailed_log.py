...
def find_same_day_window_start(candles: list[dict], i: int, candle_dates: list, max_window: int) -> int:
    ...
    the SAME calendar day as candle i. Used to fix a real, confirmed
    bug: the volatility window previously crossed overnight gaps,
    letting a single yesterday-close -> today-open jump get treated as
    one giant intraday return. Verified directly against a real trade
    (strike 23250, modeled premium Rs 383.15 vs the real Rs 221 market
    price) -- the inflated volatility from an overnight gap was
    responsible for the ~73% overpricing.
    """
...
def main():
    ...
    print("\n" + "=" * 70)
    print("SUMMARY: VWAP-filtered strategy (with the fixed same-day-only volatility model)")
    print("=" * 70)
    if summary_with_vwap:
        print(f"  Trades: {summary_with_vwap['trades']}")
        print(f"  Win Rate: {summary_with_vwap['win_rate']}%")
        print(f"  Net P&L: Rs {summary_with_vwap['total_net']:,.2f}")
        print(f"  Net Return: {summary_with_vwap['net_return_pct']}%")

    with open(OUTPUT_DIR / "vwap_5yr_summary.json", "w", encoding="utf-8") as f:
        json.dump(summary_with_vwap, f, indent=2)
    log(f"\nSummary written to {OUTPUT_DIR / 'vwap_5yr_summary.json'}")
