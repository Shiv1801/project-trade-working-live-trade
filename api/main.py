...
from engine.execution.trade_dispatcher import execute_exit, execute_entry, is_live_trading_active_for
...
from engine.risk_engine.position_sizing import compute_position_size, LOT_SIZES, get_effective_capital
...
async def position_size_signal(index_id: str, available_capital: float | None = None,
    ...
    available_capital: FIXED A REAL BUG -- this previously defaulted
    to a hardcoded Rs 100,000 and was never actually replaced with the
    real, effective capital, regardless of paper/live mode or real
    Fyers balance. Now defaults to None, and when None, is resolved
    via get_effective_capital(index_id) -- the real paper capital when
    paper trading, or the genuine live Fyers balance when live trading
    is actually active for this index. An explicit value can still be
    passed (e.g. for what-if testing), but real trading decisions
    always flow through the real effective capital by default.
    """
    index_id = index_id.upper()
    if available_capital is None:
        available_capital = get_effective_capital(index_id)
...
class TestEntryRequest(BaseModel):
    confirm_real_money: bool
    direction: str


@app.post("/positions/test-entry")
async def manual_test_entry(request: TestEntryRequest):
    """
    Places exactly ONE lot of a real, current NIFTY50 ATM option, IF
    AND ONLY IF both real-trading safety gates (master switch + index
    mode) are already on. Deliberately bypasses the confluence gate --
    this is a MECHANISM test (does a real order actually place and
    fill against this account), not a signal/strategy test -- but
    routes through the exact same trade_dispatcher.execute_entry every
    real auto-entry uses, so a success here is a genuine confirmation.

    confirm_real_money must be explicitly True in the request body --
    no default, so this can never fire from an empty/malformed request.
    """
    if not request.confirm_real_money:
        return {"status": "error", "message": "confirm_real_money must be explicitly true to use this endpoint."}
    if request.direction not in ("long_call", "long_put"):
        return {"status": "error", "message": "direction must be 'long_call' or 'long_put'."}

    index_id = "NIFTY50"
    live_active = is_live_trading_active_for(index_id)

    if not live_active:
        return {
            "status": "error", "live_trading_active": False,
            "message": "Real trading is NOT currently active for NIFTY50 (master switch and/or index mode is off). "
                       "This endpoint only places a REAL order when both gates are already on.",
        }

    token = _read_token()
    if not token:
        return {"status": "error", "live_trading_active": True, "message": "No Fyers token found. Log in via the app first."}

    parsed_chain = _parse_chain_from_shared_state(index_id)
    if not parsed_chain or not parsed_chain.get("strikes"):
        return {
            "status": "error", "live_trading_active": True,
            "message": "⚠ LIVE TRADING IS ACTIVE, but no option chain data available right now -- "
                       "market may be closed, or the chain poller hasn't run yet. NO ORDER WAS PLACED.",
        }

    option_type = "CE" if request.direction == "long_call" else "PE"
    strikes_sorted = sorted(parsed_chain["strikes"], key=lambda r: r["strike"])
    atm_row = strikes_sorted[len(strikes_sorted) // 2]
    strike = atm_row["strike"]
    side_data = atm_row.get(option_type)
    if not side_data or not side_data.get("ltp") or not side_data.get("symbol"):
        return {"status": "error", "message": f"No usable {option_type} data (missing symbol or LTP) for strike {strike} right now."}

    fyers_symbol = side_data["symbol"]
    current_price = side_data["ltp"]
    lot_size = LOT_SIZES.get(index_id)

    result = execute_entry(
        access_token=token, index_id=index_id, fyers_symbol=fyers_symbol,
        direction=request.direction, strike=strike, option_type=option_type,
        lots=1, lot_size=lot_size, simulated_or_live_price=current_price,
    )

    return {
        "status": "ok" if result["executed"] else "error",
        "live_trading_active": True,
        "message": f"Test entry {'succeeded' if result['executed'] else 'FAILED'} -- mode={result['mode']}, "
                   f"strike={strike} {option_type}, 1 lot ({lot_size} units) @ ~₹{current_price}",
        "result": result,
    }
...
async def paper_enter(index_id: str, available_capital: float | None = None,
                       risk_per_trade_pct: float = 2.0, hard_sl_pct: float = 27.0,
                       min_confidence: float = 0.5):
    """
